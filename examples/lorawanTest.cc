...
main (int argc, char *argv[])
{
  ...
  // Set up logging
  LogComponentEnable ("test1", LOG_LEVEL_ALL);
  ...
  for (NodeContainer::Iterator it_gw = gateways.Begin (); it_gw != gateways.End (); ++it_gw){
    Ptr<Node> object = *it_gw;
    Ptr<MobilityModel> gw_mobility = (*it_gw)->GetObject<MobilityModel> ();
    for (NodeContainer::Iterator it_ed = endDevices.Begin (); it_ed != endDevices.End (); ++it_ed){
      Ptr<Node> object = *it_ed;
      Ptr<MobilityModel> ed_mobility = (*it_ed)->GetObject<MobilityModel> ();
      std::cout << channel->GetRxPower (14, ed_mobility, gw_mobility) << " ";
    }
    std::cout << std::endl;
  }
  for (NodeContainer::Iterator it_gw = gateways.Begin (); it_gw != gateways.End (); ++it_gw){
  ...
  // std::cout << tracker.CountPhyPacketsPerGw (Seconds (0), appStopTime + Hours (1), nDevices) << std::endl;
